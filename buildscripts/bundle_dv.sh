...
cp flavors/dv.sh scripts/ffmpeg.sh

# --------------------------------------------------

./build.sh

zip -r debug-symbols-dv.zip prefix/*/lib
...
zip -r dv-arm64-v8a.jar      lib/arm64-v8a/*.so
zip -r dv-armeabi-v7a.jar    lib/armeabi-v7a/*.so
zip -r dv-x86.jar            lib/x86/*.so
zip -r dv-x86_64.jar         lib/x86_64/*.so

md5sum *.jar

cd ../../../../../../..

mkdir -p artifacts/dv
cp deps/media-kit-android-helper/app/build/outputs/apk/release/dv-*.jar artifacts/dv/
